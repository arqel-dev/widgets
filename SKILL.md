...
- `Filters\Filter` (abstract) — `(string $name)`; factory `Filter::make($name)`. Setters `label(string)`, `default(mixed)`. Subclasses declaram `$type` + `$component` e implementam `getTypeSpecificProps(): array`. `toArray()` emite `{name, type, component, label, default, ...}` onde `default` passa por `resolveDefault()` (hook protegido que subclasses fazem override para massajar o valor — e.g. `DateTimeInterface` ↦ string). `getResolvedDefault()` é o accessor público que devolve o **mesmo** valor serializável que `toArray().default` — usado pelo `Dashboard` ao semear os filter defaults (não `getDefault()`, que devolveria o objeto cru). Label fallback via `Str::of($name)->snake()->replace('_',' ')->title()`.
- `Filters\DateRangeFilter` (final) — `type='date_range'`, `component='DateRangeFilter'`. `defaultRange(?DateTimeInterface, ?DateTimeInterface)` armazena os endpoints como `DateTimeInterface` mas faz override de `resolveDefault()` para **serializar o default como strings `Y-m-d`** (`{from, to}`), para o controle `<input type="date">` do React conseguir popular — um `DateTimeInterface` cru não é `JsonSerializable` e vazaria o cast `{date, timezone_type, timezone}` (#165).
